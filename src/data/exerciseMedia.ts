...
//
// `frames` is 1 or 2 image paths (under public/exercises/<id>/) — two frames crossfade to
// suggest motion between the start and end position; one frame is shown static. Paths are
// root-relative (no leading slash) and get the app's base path prefixed on read, so this
// still resolves correctly when deployed under a subpath (e.g. a GitHub Pages project site).

export interface ExercisePhotoMedia {
  frames: [string] | [string, string]
}

const RAW_EXERCISE_PHOTOS: Record<string, ExercisePhotoMedia> = {
...
export function getExercisePhoto(exerciseId: string): ExercisePhotoMedia | undefined {
  const raw = RAW_EXERCISE_PHOTOS[exerciseId]
  if (!raw) return undefined
  return { frames: raw.frames.map(withBase) as ExercisePhotoMedia['frames'] }
}
